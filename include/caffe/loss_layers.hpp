...
/**
 * @brief Computes the accuracy for an angle regression task.
 */
template <typename Dtype>
class AngleAccuracyLayer : public Layer<Dtype> {
 public:
  explicit AngleAccuracyLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top) {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_ANGLE_ACCURACY;
  }
 ...
 protected:
  ...
  /// @brief Not implemented -- AngleAccuracyLayer cannot be used as a loss.
  virtual void Backward_cpu(const vector<Blob<Dtype>*>& top,
      const vector<bool>& propagate_down, vector<Blob<Dtype>*>* bottom) {
    for (int i = 0; i < propagate_down.size(); ++i) {
      if (propagate_down[i]) { NOT_IMPLEMENTED; }
...
class CosineLossLayer : public LossLayer<Dtype> {
 public:
  ...
  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_COSINE_LOSS;
  }

  /**
   * Unlike most loss layers, in the CosineLossLayer we can backpropagate
   * to both inputs -- override to return true and always allow force_backward.
   */
  virtual inline bool AllowForceBackward(const int bottom_index) const {
    return true;
